  ...
  async invite(email, admin = false) {
    ...
    if (user === null) {
      ...
      if (!process.env.DEV) {
        ...
        console.log(
          `[d] Invite token for ${user.email}: "${user.credentials.resetToken}"`
        );
      }
  ...
  async newReset(email) {
    try {
      ...
      if (user && user.status.registered) {
        ...
        console.log(
          `[w] Got Password reset request for non-activated account: ${email}`
        );
      } else {
        console.log(
          `[w] Got Password reset request for non-existing email: ${email}`
        );
      }
    } catch (err) {}
  }
